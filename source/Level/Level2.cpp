...
void LoadLevel2()
{
	...
	TransformComponent* stageTransformComponent = registry.AddComponent<TransformComponent>(stage);
	...
	stageCo->model = LoadModel("HellhoundDummy_PH.mdl");
	pmc->model = LoadModel("PlayerPlaceholder.mdl");
	(*pac) = AnimationComponent();

	stageTransformComponent->scaleX = 50.0f;
	stageTransformComponent->scaleY = 1.f;
	stageTransformComponent->scaleZ = 20.0f;
	stageTransformComponent->positionY = -.5f;
