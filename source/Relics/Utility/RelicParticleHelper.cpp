...
void ParticleAtEntityLocationFollow(const EntityID& origin, const float duration, void* funcStart)
{
	...
	registry.AddComponent<ParentComponent>(percyJackson, origin, 1337);
	registry.AddComponent<ParentControlComponent>(origin, 1337);

	// Add a timed event for the particles
	AddTimedEventComponentStartContinuousEnd(percyJackson, 0, funcStart, _CheckLiving, duration, _RelicParticleDespawn);
