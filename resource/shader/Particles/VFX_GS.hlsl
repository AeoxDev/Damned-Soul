cbuffer CameraBuffer : register(b1)
{
    float4 cameraPosition;
    matrix view;
    matrix projection;
}

struct GS_IN
{
    float4 position : SV_POSITION;
    float4 worldPosition : POSITION;
    float4 rgb : RGB;
    float rotationZ : ROTATIONZ;
    float size : SIZE;
};

struct GS_OUT
{
    float4 position : SV_POSITION;
    float4 rgb : RGB;
};

[maxvertexcount(6)]
void main(
	triangleadj GS_IN inval[6],
	inout TriangleStream<GS_OUT> retval
)
{
    float3 look = cameraPosition.xyz - inval[0].worldPosition.xyz;
    //look.y = 0.f;
    look = normalize(look);
    
    
    // TODO: Up vector needs to be calculated independantly
    float3 up = float3(0.f, 1.f, 0.f);
    float3 right = cross(up, look);
    
    
    
    // Now construct the billboard, starting with positions
    float3 verts[6];
    verts[0] = inval[0].worldPosition.xyz - right * (inval[0].size / 2) - up * (inval[0].size / 2); // Get bottom left vertex
    verts[1] = inval[1].worldPosition.xyz - right * (inval[0].size / 2) + up * (inval[0].size / 2); // Get top left vertex
    verts[2] = inval[2].worldPosition.xyz + right * (inval[0].size / 2) - up * (inval[0].size / 2); // Get bottom right vertex
    verts[3] = inval[3].worldPosition.xyz + right * (inval[0].size / 2) - up * (inval[0].size / 2); // Get bottom right vertex
    verts[4] = inval[4].worldPosition.xyz - right * (inval[0].size / 2) + up * (inval[0].size / 2); // Get top left vertex
    verts[5] = inval[5].worldPosition.xyz + right * (inval[0].size / 2) + up * (inval[0].size / 2); // Get top right vertex

    float4x4 viewProj = mul(view, projection);

    // Now append it to output
    GS_OUT retappend;
    [unroll]
    for (int i = 0; i < 6; i++)
    {
        retappend.position = mul(float4(verts[i].xyz, 1.0f), viewProj); // times rotationZ here?
        // INSERT rotate on Z-axis
        //retappend.position = retappend.position / retappend.position.w;
        retappend.rgb = inval[0].rgb;


        retval.Append(retappend);

        if (i == 2)
            retval.RestartStrip();
    }
}