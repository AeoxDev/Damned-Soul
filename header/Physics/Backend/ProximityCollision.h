...
void ProximityMove(ProximityPoint& p1, ProximityPoint& p2, 
		float& x, float& z, int& clockwise); //Find normal of line 1 -> 2 and Move entity along the normal based on magnitude.

void ProximityStepChart(ProximityPoint& A, ProximityPoint& B, ProximityPoint& C, 
		float& x, float& z, float& px, float& pz, int& clockwise); //Go through the steps of how to handle the collision if entity is on the wrong side of both lines.