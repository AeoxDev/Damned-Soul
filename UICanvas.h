...
protected:
	...
	void AddButton(PoolPointer<UI>& ui, const std::string& imageFile, std::wstring buttonString, std::function<void()> onClick,
	...
	void AddImage(PoolPointer<UI>& ui, const std::string& name, std::string& file, DirectX::SimpleMath::Vector2 position,
		DirectX::SimpleMath::Vector2 scale = { 1.0f, 1.0f }, float rotation = 0.0f, bool visibility = true, float opacity = 1.0f);
	
	void AddText(PoolPointer<UI>& ui, const std::string& name, std::wstring& text, DirectX::SimpleMath::Vector2 position,
